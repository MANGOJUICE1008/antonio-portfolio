...
// pushed to the bottom of the archive until renamed.
//
// The newest issue is shown expanded by default; every other issue opens
// in a modal viewer (same lightbox pattern as the photo gallery) when
// clicked. Every issue also has a direct download link.
// ============================================================
...
export default function NewsletterPage() {
  ...
  const latest = hasNewsletters ? newsletters[0] : null;
  const older = hasNewsletters ? newsletters.slice(1) : [];
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Latest issue — shown expanded by default */}
            ...
            {older.map((issue) => (
              ...
              >
                <div className="flex-grow min-w-0">
                  <h3 className="font-bold text-slate-900 truncate">{issue.title}</h3>
                  <p className="text-xs text-slate-400 font-mono mt-1">
                    {issue.date} · {issue.sizeLabel}
                  </p>
                </div>
            ...
            <div className="p-4 flex items-center justify-between gap-3 border-b border-slate-100 flex-shrink-0">
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-400 font-mono mt-0.5">
                  {viewing.date} · {viewing.sizeLabel}
                </p>
              </div>
              <a
